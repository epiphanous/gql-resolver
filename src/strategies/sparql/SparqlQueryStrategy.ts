...
export class SparqlQueryStrategy extends QueryStrategy {
  ...
  private endCursorPerSubject: Map<string, string> = Map<
    string,
    string
  >().asMutable();
      ...
      return Object.keys(entry).reduce(
        (acc: { [key: string]: any }, key) => {
          ...
          if (key === 'totalCount') {
            ...
            hasNextPage =
              (optFirst.value && Number(count) > (optFirst.value as number)) ||
              false;
            hasPreviousPage =
              (optLast.value && Number(count) > (optLast.value as number)) ||
              false;
          }
          acc[key] = lit.value;
  ...
  public moreThanOneSubjectId() {
    return (
      this.plan.grandParentPlan().get() &&
      this.plan.grandParentPlan().get()!.parent &&
      this.plan
        .grandParentPlan()
        .get()!
        .parent!.getSubjectIds().size > 1
    );
  }
   ...
   */
  public constructPageInfoQuery(
    injectable: boolean = false,
    parentConstraint: string = ''
  ): string {
    if (
      this.moreThanOneSubjectId() &&
      (this.plan.processedArgs.before || this.plan.processedArgs.after) &&
      !injectable
    ) {
    ...
    return `
        ...
        WHERE {
          ${
            injectable
              ? `VALUES ?parentId { <${parentConstraint}> } ${this.addGeoNearbyConstraint()}`
  ...
  public executeQuery(query: string): Promise<any> {
    ...
    return new Promise(resolve => {
      this.fetcher.fetchBindings(this.endpoint, query).then(stream => {
  ...
  public processData([rows, errors]: any[]) {
    ...
    queryResult.data = this.mapResultsObjectToOrderedMap(
      this.mapLiteralResultsToObjects(rows)
    );
  ...
  public injectPageInfo(pageInfo: Array<{ [key: string]: string }>) {
    pageInfo.forEach((pageInfoObject: { [key: string]: string }) => {
      this.plan
        .grandParentPlan()
        .get()!
        .result.data.merge(OrderedMap({ [pageInfoObject.s]: pageInfoObject }));
    });
  }

  // self-explanatory
  public mapLiteralResultsToObjects(
    results: Array<{ [key: string]: Literal }>
  ) {
  ...
  public mapResultsObjectToOrderedMap(
    results: Array<{ [key: string]: string }>
  ): OrderedMap<string, any> {
    const fieldsAliases = this.fields.map(field =>
      field.alias.getOrElse(field.name)
    );
    const remapToOnlyRequestedFields = (listOfResultObjects: {
      [key: string]: any;
    }) =>
      listOfResultObjects.map((obj: { [key: string]: any }) =>
        getOnlyRequestedFields(obj)
      );
    const getOnlyRequestedFields = (singleResultObjectOrList: {
      [key: string]: any;
    }): { [key: string]: any } => {
      console.log(
        'singleResultObjectOrList',
        JSON.stringify(singleResultObjectOrList, null, 2)
      );
      return Object.assign(
        {},
        ...Object.keys(singleResultObjectOrList).map(key => {
    ...
    return List(results)
      .groupBy(row =>
        this.hasProperParent() || this.plan.greatGrandParentPlan().value
          ? row.parentId
          : row.s
      )
      .toOrderedMap()
      .map(x =>
        List.isList(x)
          ? x.size <= 1
            ? OrderedMap(getOnlyRequestedFields(x.get(0)))
    ...
    return new Promise(async (resolve, _) => {
      const results = this.processData(
        await this.executeQuery(
          this.constructDataQuery(objectType, args, projections)
        )
      );
      if (this.plan.isConnectionEdgesPlan()) {
        const pageInfo = this.processPageInfo(
          await this.executeQuery(this.constructPageInfoQuery())
        );
  ...
  protected addFilters() {
    return this.plan.processedArgs.filter
      .map(filter => {
        ...
        const separator =
          expr instanceof GQLObjectQueryModifierDisjunction ? '|| ' : '&& ';
          ...
          return expr.values
            .map(filterMap => {
              ...
              return filterMap
                .keySeq()
                .map(
                  key => `${key.expression} = ${filterMap.get(key)!.expression}`
                )
                .get(0);
            })
            .join(separator);
  ...
  protected addParentConstraints() {
    if (
      ...
      (this.plan.grandParentPlan().value &&
        this.plan.grandParentPlan()!.value!.name === 'gn_nearby')
    ) {
      ...
      const parentConstraint =
        this.plan.grandParentPlan().get()!.name === 'gn_nearby'
