...
export default class GQLBoostersBuilder extends GQLObjectQueryModifierBuilder {
    ...
    public processFollowsUser(context: QMP.FollowsUserBoostContext) {
        const boost = this.processNumericLiteral(context.numericLiteral()).expression;
        const user = Number(this.processIriRefOrVarRef(context.iriRefOrVarRef()).expression);
