...
import { GQLError, IResolvedResult } from '.';
...
export class QueryResult {
  ...
  constructor(
    ...
    duration = 0,
    data = List<OrderedMap<string, QueryValue>>(),
  ...
  public addItem(item: OrderedMap<string, QueryValue>) {
    this.addItems([item]);
  }

  public addItems(items: Array<OrderedMap<string, QueryValue>>) {
    this.data = this.data.withMutations(d =>
      items.forEach(item => d.push(item))
    );
  }

  public addError(error: GQLError) {
    this.addErrors([error]);
  }

  public addErrors(errors: GQLError[]) {
    this.errors = this.errors.withMutations(mutable => {
      errors.forEach(e => mutable.push(e));
    });
  }

  public toJS(): IResolvedResult {
    const js = this.data.toJSON().map(om => {
      ...
      return om.reduce((obj, value, key) => {
        if (value instanceof QueryResult) {
          ...
          // List<Scalar>
          obj[key] = (value as List<Scalar>).toArray();
        } else {
          obj[key] = value;
        }
        return obj;
    ...
    const data = this.isList ? js : js.length > 0 ? js[0] : null;
    const result: IResolvedResult = { data };
    const errors = this.errors.toJSON();
    if (errors.length > 0) {
  ...
  public merge(that: QueryResult, byKeys = List<string>()) {
    if (!that.data.isEmpty()) {
      if (this.data.isEmpty()) {
        ...
        // this.data =
        const x = this.data
          .zipAll(that.data as List<OrderedMap<string, QueryValue>>)
          .map<OrderedMap<string, QueryValue>>(([r1, r2]) => {
            const d1 = r1 || OrderedMap<string, QueryValue>();
            const d2 = r2 || OrderedMap<string, QueryValue>();
            return d1.merge(d2);
          });
        this.updateMeta();
      } else {
  ...
  private updateMeta() {
    ...
    this.meta.ok = this.errors.isEmpty();
  }
