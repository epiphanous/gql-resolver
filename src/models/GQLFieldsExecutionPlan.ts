...
export class GQLFieldsExecutionPlan extends GQLExecutionPlan {
  public projectionOrder: List<NameAliais>;
  ...
  public shouldHaveOnlyOne(key: string) {
    return (
      Set([ID_BINDING, TYPENAME_BINDING]).contains(key) ||
  ...
  public execute(
    ...
    // TODO: swap back to List<OrderedMap<...>> if needed
    ...
    const results: List<OrderedMap<string, any>> = this.strategies(subjectIris)
      ...
      .map((listOfMaps: List<Map<string, any>>) => {
        ...
        const specialFields: Map<string, List<any>> = Map<any>([
          [
            ID_BINDING,
            Option.of(mapOfLists.get(SUBJECT_BINDING)).getOrElse(List<any>()),
          ],
          [
            TYPENAME_BINDING,
            Option.of(mapOfLists.get(TYPENAME_BINDING)).getOrElse(List<any>()),
          ],
        ]).map(x => List(this.iriWithNameToPrefixedString(x.toString)));

        const mapOfListsWithSpecialFields: Map<
          string,
          List<any>
        > = mapOfLists.concat(specialFields);

        const unsorted: Map<
          string,
          any
        > = mapOfListsWithSpecialFields
          .filterNot(v => v.isEmpty())
          .map((v, k) => (this.shouldHaveOnlyOne(k) ? [k, v.first()] : [k, v]));
